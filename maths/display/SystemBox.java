...
public class SystemBox extends Box {
    ...
    /**
     * transforme en une expression à afficher en gardant les références aux
     * variables initiales
     *
    ...
    /**
     * retourne une copie de l'expression e avec des variables identiques à
     * celles de la liste
     *
     * @param e
     * @param vars
    ...
    public static Expression copyref(Expression e, List<Expression> vars) {
        ...
        if (e.op == VAR) {
            if (vars.contains(e)) {
                ret = (Expression) vars.get(vars.indexOf(e)).R;
            }
            return ret;
        }
        Object nL = e.L, nR = e.R;
        if (nL instanceof Expression) {
            nL = copyref((Expression) nL, vars);
        }
        if (nR instanceof Expression) {
            nR = copyref((Expression) nR, vars);
    ...
    public static SystemBox exprBox(Expression expr, Expression displays, GeomPanel GPanel, int size,
        ...
        if (e.op < IS) {
            if (e.op == NUL) {
                if (e == Relation.TRUE) {
                    box = new SystemBox(new TextBox("VRAI", g, size), new int[]{1, 1, 1, 1});
                } else if (e == Relation.FALSE) {
                    box = new SystemBox(new TextBox("FAUX", g, size), new int[]{1, 1, 1, 1});
                }
                return box;
            ...
            // TBOX(a,b,JUX(c,d),x) -> JUX(JUX(TBOX(a,b),JUX(c,d)),x)
                ...
                try {
                    ...
                    for (int i = 1; i < coors.length; i++) {
                        ...
                        if (params.length == 5) {
                            rang = ((BigInteger) params[4].L).intValue();
                        }
                ...
                if (idx != -1) {
                    fig = (ArrayList) (decofigs.get(idx)).R;
                }
                DrawBox deco;
                ...
                Expression[] coors = (Expression[]) e.R;
                if ("VBOX".equals(e.L) && coors[0].op == SEQ) {
                ...
                if ("VBOX".equals(e.L) || "VLBOX".equals(e.L)) {
                    ...
                    Box nameBox;
                    if (e.L instanceof String) {
                        nameBox = new TextBox(e.L, g, size);
                    } else {
                        nameBox = exprBox((Expression) e.L, displays, GPanel, size, false, hot);
                    }
                    box.add(nameBox, "OUEST");
                }
            }
        } else {
            box = binaryBox(e, displays, GPanel, size, false, hot);
        }
        if (brackets) {
            box = box.getBrackets(bracketType, size, GPanel);
        }
    ...
    public static SystemBox binaryBox(Expression e, Expression displays, GeomPanel GPanel, int size,
        ...
        if (e.op == DIV) {
            ...
            if (l.op == DIV) {
                lbox = lbox.getBrackets("  ", size, GPanel);
            }
            if (r.op == DIV) {
                rbox = rbox.getBrackets("  ", size, GPanel);
            }
            int mw = lbox.getBoxMetrics().width;
            if (mw < rbox.getBoxMetrics().width) {
                mw = rbox.getBoxMetrics().width;
            }
    ...
    private SystemBox getBrackets(String type, int size, GeomPanel GPanel) {
        ...
        if (n > 0) {
            if (h > 21) {
                box.add(0, P0, P1, new DrawBox(box, type.substring(0, 1)));
            } else {
        ...
        if (n == 2) {
            if (h > 21) {
                box.add(0, P1, P0, new DrawBox(box, type.substring(1)));
            } else {
    ...
    /**
     * ajoute horizontalement ou verticalement une boîte à la dernière boîte ou
     * s'il n'y a rien équivaut à SystemBox(box)
     *
        ...
        if (frame != null) { // cadre autour
            ...
            if (frame[2] != -1) { // dessin du cadre
                ...
                if (radius == -1) {
                    ...
                    g.fillRoundRect(x - margin, y - margin, w + 2 * margin, h + 2 * margin, radius, radius);
                }
                g.setColor(save);
            }
            if (frame[0] >= 0) { // dessin du cadre
                if (radius == -1) {
                    g.drawRect(x - margin, y - margin, w + 2 * margin, h + 2 * margin);
                } else {
                    g.drawRoundRect(x - margin, y - margin, w + 2 * margin, h + 2 * margin, radius, radius);
                }
            }
        }
        if (active != 0) {
