...
/**
 * Décrit une relation entre deux expressions mathématiques : = , < , > logiques
 * : non, et, ou , implique
 *
...
public class Relation extends Expression {
    ...
    public static Expression build(int op, Object e0, Object e1) throws ClassCastException {
        ...
        switch (op) {
            ...
            case CUP:
            case CAP:
                g = (g0 || s0.equals(ENSEMBLE)) && (g1 || s1.equals(ENSEMBLE));
                break;
            case EQU:
            case LT:
            case GT:
     ...
     * opérande(a,1)->3+2 ou opérande(s,[3,5])->2, donc remplacer par la
     * variable `opérande(s,[3,5])`
     *
    ...
    /**
     * met un préfixe (_ ou $) devant chaque variable d'une Expression qui n'est
     * pas répertoriée dans vars si newvar == false, ou au contraire ajoute un
     * préfixe pour créer une nouvelle variable (appelé uniquement par la
     * méthode addvars(defs, start, level)
     *
     * @return le schéma
    ...
    public static Expression schema(Expression p, String pre, AbstractList<Expression> vars, int s) {
        ...
        if (p.op == VAR && !((String) p.L).startsWith("?")) {
            if (p.R != null && var("constant").equals(p.R)) {
                return p;
            }
            for (int i = constants.length - 1; i > -1; i--) {
                if (constants[i].equals(p.L)) {
                    return p;
                }
            }
            ...
            if (p.L instanceof Expression) {
                p.L = schema((Expression) p.L, pre, vars, s);
            }
            if (p.R == null) {
                return build(p.op, p.L, null);
            } else if (p.R instanceof Expression) {
                return Expression.build(p.op, p.L, schema((Expression) p.R, pre, vars, s));
                // construit une relation
            } else if (p.R instanceof Expression[]) {
    ...
    /**
     * vérifie si l'expression e est conforme au modèle p qui se transforme en e
     * si vrai
     *
    ...
    public static boolean match(Expression e, Expression p, ArrayList<Expression> vars, ArrayList<Expression> exprs) {
        ...
        if (p.op == VAR && (((String) p.L).startsWith("_") || ((String) p.L).startsWith("?"))) {
            ...
            } else if (e.op != SEQ && !p.L.equals(e.L)) {
                return false;
            }
            ...
            ret = p.equals(e); // entier ou décimal
        }
        return ret;
    }
    ...
    public boolean valueOf(ArrayList<Expression> defs, int start, int level) throws Exception {
        ...
        if (op == OR || op == AND || op == IMP) {
            ...
            if (op == OR) {
                value = Rl.valueOf(defs, start, level) || Rr.valueOf(defs, start, level);
            } else if (op == AND) {
                value = Rl.valueOf(defs, start, level) && Rr.valueOf(defs, start, level);
            } else if (op == IMP) {
                value = !Rl.valueOf(defs, start, level) || Rr.valueOf(defs, start, level);
            ...
            if ((op == EQU || op == IN) && L1.op == SET && R1.op == SET) { // égalité ou inclusion d'ensembles
                ...
                if (!"{}".equals(R1.L)) {
                    ...
                    if ("{}".equals(L1.L)) {
                        for (Expression coor1 : coors1) {
                            int op1 = (brackets[0] == '[') ? GE : GT;
                            if (!((Relation) build(op1, coor1, coors2[0])).valueOf(defs, start, 1)) {
                                return false;
                            }
                            int op2 = (brackets[1] == ']') ? LE : LT;
                            if (!((Relation) build(op2, coor1, coors2[1])).valueOf(defs, start, 1)) {
                                return false;
                            }
                        }
                        return true;
                ...
                if (op == EQU) {
                    ...
                    for (int i = n - 1; i > -1; i--) {
                        if (v1.indexOf(coors2[i]) == -1) {
                            return false;
                        }
                    }
                }
                return true;
            } else if (op == EQU && L1.equals(R1)) {
                return true;
            }
