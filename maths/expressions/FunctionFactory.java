...
public class FunctionFactory {
  ...
  static String[] names = {"approche", "arrangement","calcule", "card", "choix", "compare", "compose", 
      "décompose",  "ensemble", "euclide", "mappe", "opérande", "joint", "sépare", "pgcd", "seq", "tests", 
      "var","scinde","dans", "stylefonc", "styleop"};
  static String[] methods = {"round", "permutation","compute", "card", "choice", "compare", "makeup", 
      "breakdown",  "set", "euclide", "map", "operand", "join", "split", "gcd", "seq", "tests", 
      "makevar","splitvar","contains", "func_style", "op_style"};
  static HashMap<String, Method> table = new HashMap<String, Method>(25);
  private static final String[] opStrs = {"PLUS","MOINS","MUL","DIV","EXP","EGAL","INF","SUP","INFEG",
      "SUPEG","NON","OU","ET","DONC","UNION","INTER"};
  private static final int[] ops = {SUM,SUB,MUL,DIV,EXP,EQU,LT,GT,LE,GE,NOT,OR,AND,IMP,CUP,CAP};
  ...
  public Expression join(Expression[] coors) {
    ...
    Expression[] res = new Expression[0], n, u;
    boolean set = coors[0].op == CUP;
    if(set) coors = new Expression[]{(Expression)coors[0].L, (Expression)coors[0].R};
      for (Expression coor : coors) {
          if (coor == null || coor.op == NUL) {
              continue;
          } else if (set && coor.op == SET && "{}".equals(coor.L)) {
              n = (Expression[]) coor.R;
          } else {
              // suites
              n = (coor.op == SEQ && !"i".equals(coor.L)) ? (Expression[]) coor.R : new Expression[]{coor};
          }
  ...
  public Expression makevar(Expression[] coors) {
    ...
    if(coors.length == 1 && coors[0].op == SEQ) {
      ...
      for (Expression coor : coors) {
          s += (coor.op == VAR) ? (String) coor.L : coor.printout(false);
      }
    return var(s);
  }
    ...
    if(ret == Relation.FALSE && coors[1].R instanceof Expression) {
      ...
      Expression[] items = (Expression[])coors[1].R;
        for (Expression item : items) {
            ret = contains(new Expression[]{coors[0], item});
