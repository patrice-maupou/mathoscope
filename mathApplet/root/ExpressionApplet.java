...
public class ExpressionApplet extends java.applet.Applet implements Runnable {
   ...
   * @param value expression décrivant le point d'ancrage de l'entrée, exemple : {point(x,y),label(e,..)}
        ...
        try {
          ...
          if (i != -1) {
            ...
            for (int j = 0; j < list.length; j++) {
              Expression f = (Expression) list[j].L;
              f.R = (Expression) list[j].R;
  ...
  public void setTools() {
    ...
    try {
      Graphics g = GPanel.getGraphics(); // les dessins à afficher
      ...
      for (int i = 0; i < symbols.size(); i++) {
        ...
        //*/
        /*
        Expression e = symbols.get(i).codeDisplay(DISPLAYS);
        SystemBox box = SystemBox.exprBox(e, DISPLAYS, GPanel, 12, false, false);
        //*/
  ...
  /**
   * l'entrée au clavier est dessinée directement sur le GPanel
   * la liste root contient les parents successifs à partir du premier,
   * active, qui pointe l'élément contenant le caret, l'actualisation se
   * fait dans le thread par la méthode run.
   * exemples : JUX(caret,32) , SEQ(3,caret,7), caret
   */
  ...
  /** Valide le nouvel arbre expression en reconstuisant à partir de active et change l'affichage du dernier point
  ...
  public ArrayList<String[]> setExercice(int numero) throws Exception {
    ...
    int k = textExo.indexOf("énoncé :"), start = 0, end = 0;
    while (k != -1) {
      ...
      */
      ...
      if (!params[CHECK].equals("{}")) { // évaluations à afficher
        ...
        for (int i = 0; i < defs.length; i++) {
          mathFrame.addDefinition(defs[i], 2 * Partie + 1, "");
        }
        */
      }
    } catch (Exception e) {
  ...
  public void setDecorations(ArrayList<Expression> decorations) {
    ...
    for (int i = 0; i < decorations.size(); i++) {
      ...
      try {
        if (name.op == SET && (name.L instanceof Expression)) {// {F(x)->image : HBOX..}
          ...
          for (int k = 0; k < list.length; k++) { // liste d'instructions graphiques
            if (list[k].op == SET) {
              elems = (Expression[]) list[k].R;
            }
            else {
              elems = (new Expression[]{list[k]});
            }
    ...
    if (displays != null) { // ajoute les nouveaux symboles
      for (int j = 0; j < displays.length; j++) {
        ...
        Expression display = (Expression) displays[j].L; // l'expression transformée
        ...
        Expression displaydef[] = (Expression[]) displays[j].R;
        for (int l = 0; l < displaydef.length; l++) {
          if (displaydef[l].R.equals(Expression.var("image"))) {
            Expression symbol = (Expression) displaydef[l].L;  // l'expression initiale
            if (visible) {
              symbols.add(symbol);
            }
  ...
  private ArrayList<GObject> addFig(ArrayList<GObject> gobjs, int shift, Expression fig, GBase base)
    ...
    for (int i = 0; i < elems.length; i++) {
      GObject GObj = null;
      Object type = elems[i].L;
      if (elems[i].op == SEQ) {//séquence de séquences, il faut une récursivité
        gobjs = addFig(gobjs, shift, elems[i], lastBase);
      }
      else if (type.equals("cliquer")) {
        clicsBase = lastBase;
        grElems = (Expression[]) elems[i].R;
        addVar("clics", grElems[0]);
        nbclics = ((BigInteger) grElems[0].L).intValue();
      }
      else if (type.equals("clip")) { // nouveau cadre
        grElems = (Expression[]) elems[i].R;
      ...
      else if (type.equals("FIN")) { // fin des interactions liées à plusieurs objets
        Expression[] coors = (Expression[]) elems[i].R;
        for (int j = 0; j < coors.length; j++) {
          String objname = (String) coors[j].L;
        ...
        if ("TRAINER".equals(type) && elems[i].op == FUNC) {
          Expression[] coors = (Expression[]) elems[i].R;
          String objname = (String) coors[0].L;
          Expression value = coors[1].applyDefs(definitions, 0, 50, null); // valeur initiale
          addVar(objname, value);
        ...
        else if (elems[i].op == IS) { // variable modifiable ou suite
          Expression var = (Expression) type;
          userActions.spotsdefs.add(elems[i]); // on ajoute la définition aux interactions
          elems[i].define(definitions, true);
          if (var.op == VAR) {
            ...
            try {
              ...
              if (place == null) { // création éventuelle d'un objet graphique
                GObj = createGObject(value, lastBase);
                if (GObj != null) {
                  userActions.spots.put(name, new Integer(gobjs.size()));
                }
              }
              else if (GObj != null) {
          ...
          GObj = createGObject(elems[i], lastBase);
        }
        if (GObj != null) {
          GObj.setView();
          gobjs.add(GObj);
  ...
  /** met à jour le vecteur definitions par rapport au vecteur defs (définitions globales) de
   *  MathFrame
   *  @param defs
   *  @param globalsize
   */
...
*/
  ...
  /**
   * entrée dans le cas d'une utilisation autonome (java MathFrame w h) où xdim
   * et ydim sont les dimensions de l'écran.
...
*/
  ...
  public void loadExercices(String exercices[], String intro, String defs, String decos) {
    ...
    try {
      if (!"".equals(defs)) {
        ...
        for (int j = 0; j < adefs.length; j++) {
          if (adefs[j].op == -8) {
            mathdefs.add(adefs[j]);
          }
          adefs[j].define(definitions, false);
        }
        if (!mathdefs.isEmpty()) {
    ...
    try {
      if (!decos.equals("")) {
        ...
        for (int k = 0; k < adecos.length; k++) {
          Expression deco = adecos[k];
          int l = -1;
          if (adecos[k].op == -1) { // élément graphique
            deco = (Expression) adecos[k].L;
            deco.R = (Expression) adecos[k].R;
          }
          l = decorations.indexOf(deco);
          if (l == -1) {
   ...
   *
   * @param Parameters
   *            New value of property Parameters.
   */
   ...
   * @param replace
   *            remplace l'ancien titre si vrai, ajoute si faux
   */
